# -*- coding: utf-8 -*-
"""
Created on Sun Dec 27 16:19:04 2020
...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov 26 14:30:30 2020
...
class MultipleBarPrinter_3d(QWidget):
    def __init__(self):
        self.datas = []
        super().__init__()
        self.figure = Figure()
        ...
        self.setWindowTitle("Diagramme à bandes multiples en 3D")
    ...
    def multiplebar3d(self,information):
        print(information) 
        ...
        if information[0] == 1:
            for i in range(len(information[1])):
                if information[2][2][i][0] != information[2][2][i][1]:
                    if type(is_only_number(self.datas[information[1][i]][information[2][2][i][0]:
                        ...
                        QMessageBox.information(self,"Information","La case " +
                                            str(is_only_number(self.datas[information[1][i]][information[2][2][i][0]:
                                                                         information[2][2][i][1]-1])[1]) + " de la colonne "
                    ...
                    if type(is_only_number(self.datas[information[1][i]])) == bool:
                        ...
                        QMessageBox.information(self,"Information","La case " +
                                            str(is_only_number(self.datas[information[1][i]]) + " de la colonne "
                                            +TABLES_NAMES[information[1][i]]+ "contient un caractère non numérique"))     
            ...
            if information[4][0] != "None":
                
                x =[]
                """
                if information[4][1] != information[4][2]:     
                    if type(is_only_number(self.datas[get_tab_pos(information[4][0])][information[4][1]:information[4][2]])) == bool:
                        x = to_number(self.datas[get_tab_pos(information[4][0])][information[4][1]:information[4][2]])
                
                    elif type(is_only_number(self.datas[get_tab_pos(information[4][0])][information[4][1]:information[4][2]])) == tuple:
                        QMessageBox.information(self,self,"Information","La case " +
                                            str(is_only_number(self.datas[information[4][0]][information[4][1]:information[4][2]])[1]) + " de la colonne "
                                            +TABLES_NAMES[information[4][0]]+ "contient un caractère non numérique")
                    
                else:
                   if type(is_only_number(self.datas[get_tab_pos(information[4][0])])) == bool:
                       x = to_number(self.datas[get_tab_pos(information[4][0])])
                       
                   elif type(is_only_number(self.datas[get_tab_pos(information[4][0])])) == tuple:
                       QMessageBox.information(self,self,"Information","La case " +
                                            str(is_only_number(self.datas[information[4][0]])[1]) + " de la colonne "
                                            +TABLES_NAMES[information[4][0]]+ "contient un caractère non numérique")
                    
               """ 
                labels_ = information[2][0]
                colors_ = information[2][1]
                i = 0
                yu = np.arange(len(labels_))
                for c, k in zip(colors_,yu):
                    # Generate the random data for the y=k 'layer'.
                    ys = self.datas[information[1][i]]
                    xs = np.arange(len(ys))
                    ...
                    self.ax.bar(xs, ys, zs=k, zdir='y', color=cs)
                    
                self.ax.set_yticklabels(information[2][0])
                self.ax.set_title(information[3][0]+ "                      |"+SOFTWARE_NAME)
                self.ax.set_xlabel(information[3][1])
                self.ax.set_ylabel(information[3][2])
                self.ax.set_zlabel(information[3][3])
                self.ax.set_yticks(yu)
                ...
                self.ax.set_yticklabels(information[2][0])
                self.ax.set_title(information[3][0]+ "                      |Created whith Analys")
                self.ax.set_xlabel(information[3][1])
                self.ax.set_ylabel(information[3][2])
                self.ax.set_zlabel(information[3][3])
                self.ax.set_yticks(yu)
