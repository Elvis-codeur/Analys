# -*- coding: utf-8 -*-
"""
Created on Sun Dec 27 11:29:12 2020
...
class Bar3DPrinter(QWidget):
    def __init__(self):
        ...
        self.setWindowTitle("Diagramme a bandes en trois dimensions")
    ...
    def play_fonc(self):
        ...
        if self.fonction_receiver.text() != "":
            ...
            if fonc == "bar":
                ...
                code = code +"self.ax.bar(x,"+str(arg[0])+",label = '"+str(arg[1])+"')\n"
                ...
                QMessageBox.information(self,"information","la fonction adéquate à mettre ici est  bar")
            ...
            if len(information[1]) == 3:
                ...
                if type(is_only_number(self.datas[information[1][0]])) == bool:
                    if type(is_only_number(self.datas[information[1][1]])) == bool:
                        if type(is_only_number(self.datas[information[1][2]])) == bool:
                            
                            l = len(self.datas[information[1][0]])
                            ...
                            self.datas[information[1][2]] = to_number(self.datas[information[1][2]])
                            ...
                            if limits[0] != '' and limits[1] != '':
                                if limits[0] != limits[1]:
                                    ...
                                    self.datas[information[1][0]] = self.datas[information[1][0]][limits[0]:limits[1]]
                            
                            if limits1[0] != '' and limits1[1] != '':
                                if limits1[0] != limits1[1]:
                                    try:
                                        limits1[0] = int(limits[0]) -1
                                    except:
                                        QMessageBox.information(self,"Information","La valeur de la zone de saisie pour le début n'est pas un entier")
                                    try:
                                    ...
                                    self.datas[information[1][1]] = self.datas[information[1][0]][limits1[0]:limits1[1]]
                            
                            if limits2[0] != '' and limits2[1] != '':
                                if limits2[0] != limits2[1]:
                                    try:
                                        limits2[0] = int(limits2[0]) -1
                                    except:
                                        QMessageBox.information(self,"Information","La valeur de la zone de saisie pour le début n'est pas un entier")
                                    try:
                                        limits2[1] = int(limits2[1])
                                    except:
                                        QMessageBox.information(self,"Information","La valeur de la zone de saisie fin n'est pas un entier")
                                
                                    self.datas[information[1][2]] = self.datas[information[1][0]][limits2[0]:limits2[1]]
                            ...
                            if width != 0:
                                ...
                                self.ax.bar3d(self.datas[information[1][0]],
                                        self.datas[information[1][1]],
                                        c,
                                        d,
                                        e,
                                        self.datas[information[1][2]],
                            ...
                            self.ax.set_title(information[2][0]+ "                      |Created with "+SOFTWARE_NAME)
                            self.ax.set_xlabel(information[2][2])
                            self.ax.set_zlabel(information[2][4])
                            ...
                            QMessageBox.information(self,"Information","La case " +
                                            str(is_only_number(self.datas[information[1][0]])[2]) + " de la colonne "
                                            +TABLES_NAMES[information[1][0]]+ "contient un caractère non numérique")
                    else:
                        QMessageBox.information(self,"Information","La case " +
                                            str(is_only_number(self.datas[information[1][0]])[1]) + " de la colonne "
                                            +TABLES_NAMES[information[1][0]]+ "contient un caractère non numérique")
                    
                else:
                    QMessageBox.information(self,"Information","La case " +
                                            str(is_only_number(self.datas[information[1][0]])[0]) + " de la colonne "
                                            +TABLES_NAMES[information[1][0]]+ "contient un caractère non numérique")
