...
class MultipleBarPrinter(QWidget):
    def __init__(self):
        self.datas = []
        super().__init__()
        self.figure = Figure()
        ...
        self.setWindowTitle("Diagramme à bandes multiples")
    ...
    def multiplebar(self,information):
        print(information) 
        ...
        if information[0] == 1:
            ...
            if information[4][0] != "None":
                ...
                gap = 0.8/len(final_data)
                x = np.array(x)


                for i, row in enumerate(final_data):
                    self.ax.bar(x + i * gap, row,
                            width = gap,
                            color = colors_[i],
                            label = labels_[i])
                ...
                self.ax.set_title(information[3][0])
                self.ax.set_xlabel(information[3][1])
                self.ax.set_ylabel(information[3][2])
