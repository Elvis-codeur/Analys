...
class MultipleBarInfoReceiver_3d(QWidget):
    def __init__(self):
        super().__init__()
        ...
        # Printer
        self.multiplebar_printer = MultipleBarPrinter_3d()
        self.a = MultipleBarPrinter_3d()
        
        self.init_UI()
        
    def init_UI(self):
        self.setWindowIcon(QIcon("analys.png"))
        self.setWindowTitle("Fenêtre de renseignement du diagramme à multiple bandes en 3D")
            ...
            if self.new_radio_button.isChecked():
                self.multiplebar([2,
                                ...
                                [self.title_receiver.text(),
                                 ...
                                 self.z_label_receiver.text()],(self.tab_combobox.currentText())])
            else:
                self.multiplebar([1,
                                choice,[labels,colors,limits],
                                [self.title_receiver.text(),
    ...
    def multiplebar(self,info):
        if info[0] == 1:
            self.multiplebar_printer.multiplebar3d(info)
            self.multiplebar_printer.show()
        elif info[0] == 2:
            info[0] = 1
            self.a.clear()
            self.a.set_datas(self.datas)
            self.a.multiplebar3d(info)
            self.a.show()
