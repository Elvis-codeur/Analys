# -*- coding: utf-8 -*-
"""
Created on Thu Nov 26 15:25:34 2020
...
from Visualisation_outils.boxplotprinter import *
from Visualisation_outils.stackbuttons import *


class BoxPlotInfoReceiver(QWidget):
    def __init__(self):
        super().__init__()
        ...
        # Printer
        self.boxplot_printer = BoxPlotPrinter()
        self.a = BoxPlotPrinter()
        
        self.init_UI()
        
    def init_UI(self):
        self.setWindowIcon(QIcon("analys.png"))
        self.setWindowTitle("Fenêtre de renseignement du diagramme à moustache")
    ...
    def other_manage(self):
        self.orientation_combobox.addItems(["Vertical","Horirontal"])
        self.notch_box.addItems(["False","True"])
        ...
        layout1.addRow("Orientation",self.orientation_combobox)
        layout1.addRow("Notch",self.notch_box)
            ...
            if self.new_radio_button.isChecked():
                self.multiplebar([2,
                                ...
                                [self.title_receiver.text(),
                                 ...
                                 self.y_label_receiver.text()],[self.orientation_combobox.currentText(),
                                                                self.notch_box.currentText()]])
            else:
                self.multiplebar([1,
                                choice,[labels,colors,limits],
                                [self.title_receiver.text(),
    ...
    def multiplebar(self,info):
        if info[0] == 1:
            self.boxplot_printer.boxplot(info)
            self.boxplot_printer.show()
        elif info[0] == 2:
            info[0] = 1
            self.a.clear()
    ...
    def set_printer_data(self,data):
        self.datas = data
        self.boxplot_printer.set_datas(data)
        
        
        
if __name__ == '__main__':
    app = QApplication(sys.argv)
    a = BoxPlotInfoReceiver()
    a.show()
    sys.exit(app.exec())      
