# -*- coding: utf-8 -*-
"""
Created on Thu Nov 26 14:27:53 2020
...
from Visualisation_outils.multiplebarprinter import *
from Visualisation_outils.stackbuttons import *

class MultipleBarInfoReceiver(QWidget):
    def __init__(self):
        super().__init__()
        ...
        # Printer
        self.multiplebar_printer = MultipleBarPrinter()
        self.a = MultipleBarPrinter()
        
        self.init_UI()
        
    def init_UI(self):
        self.setWindowIcon(QIcon("analys.png"))
        self.setWindowTitle("Fenêtre de renseignement du diagramme à bandes multiples")
    ...
    def other_manage(self):
        ...
        self.begin_receiver.setMaximum(1000000)
        self.begin_receiver.setMinimum(0)
        self.end_receiver.setMaximum(1000000)
        self.end_receiver.setMinimum(0)
        ...
        ax_tab_layout = QHBoxLayout()
        ax_tab_layout.addWidget(self.tab_combobox)
        ax_tab_layout.addLayout(limits_receiver_layout)
        
        a = list()
        a.append("None")
        a = a + TABLES_NAMES
        self.tab_combobox.addItems(a)
        layout1 = QFormLayout()
        
        layout1.addRow("Donnée à mettre en abscisse",ax_tab_layout)
                ...
                self.multiplebar([2,
                                ...
                                [self.title_receiver.text(),
                                 ...
                                 self.y_label_receiver.text()],(self.tab_combobox.currentText(),
                                                                int(self.begin_receiver.text()),
                                                                int(self.end_receiver.text()))])
            else:
                self.multiplebar([1,
                                choice,[labels,colors,limits],
                                [self.title_receiver.text(),
    ...
    def multiplebar(self,info):
        if info[0] == 1:
            self.multiplebar_printer.multiplebar(info)
            self.multiplebar_printer.show()
        elif info[0] == 2:
            info[0] = 1
            self.a.clear()
            self.a.set_datas(self.datas)
            self.a.multiplebar(info)
            self.a.show()
            
        
    def set_printer_data(self,data):
        self.datas = data
        self.multiplebar_printer.set_datas(data)
        
        
        
if __name__ == '__main__':
    app = QApplication(sys.argv)
    a = MultipleBarInfoReceiver()
    a.show()
    sys.exit(app.exec())      
